    ...
    <Panel title="试卷列表">
      ...
      <el-table
        ...
        <el-table-column
          ...
          label="问题描述">
          <template slot-scope="{row}">
            <span v-show="row.desc">{{row.desc}}</span>
            <!-- <el-input v-show="1" v-model="row.dec"
                      @keyup.enter.native="handleInlineEdit(row)"> -->
            </el-input>
          </template>
        </el-table-column>
        ...
        <!-- <el-table-column
          ...
          label="Visible">
          <template slot-scope="scope">
            <el-switch v-model="scope.row.is_visitable"
                       active-text=""
                       inactive-text=""
  ...
  export default {
    ...
    methods: {
      ...
      goEdit (problemId) {
        ...
        this.$router.push({name: 'edit-num', params: {problemId}})
      },
      goCreateProblem () {
          this.$router.push({name: 'create-num'})
      },
      // 切换页码回调
      currentChange (page) {
      ...
      deleteProblem (id) {
        this.$confirm('是否删除改试卷', {
          type: 'warning'
        }).then(() => {
          console.log(id)
          api.deleteNum(id).then(() => [
      ...
      updateProblem (row) {
        ...
        console.log(data)
        // let arr=[]
        // data.question_list.forEach((val,i) => {
        //     arr.push(val.id)
        // });
        data.question_list=arr
        api.editTest(data).then(res => {
